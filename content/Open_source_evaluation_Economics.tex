...
To develop and sustain an open-source project effectively, 
it is essential to establish a viable revenue model that aligns with the project's goals and objectives. 
Several revenue models have emerged within the open-source ecosystem, each offering distinct advantages and challenges. 
By selecting an appropriate revenue model, project maintainers can secure funding, support ongoing development, 
and ensure the project's long-term sustainability.

\subsection{Common Business Models}

Several business models have proven successful in the open-source domain, including:
\begin{itemize}
    \item \textbf{Open core:} This model has the core code of thier software is open source and freely available, but additional features and functionalities are offered under a commercial license. (e.g., MongoDB, GitLab)
    \item \textbf{Hosting and Cloud solutions:} Some companies offer hosting services or cloud-based solutions for thier open source software, charging users for access to these services. (e.g., WordPress, Databrickes)
    \item \textbf{Support and Maintenance services:} To get revenue in this company 
\end{itemize}
