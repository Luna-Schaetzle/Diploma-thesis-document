...
\section{Open source AI Models}
\label{sec:Open_source_AI_Models}

In the field of artificial intelligence, it is very difficult to categroies the models as open source or not.

This is because some models falslie claim to be open source, but in reality, they are not true open source models.

For an AI model to be considered truely open source, it must meet the following criteria:
\begin{itemize}
    \item The model weights and architecture must be freely available to the public.
    \item The model must be licensed under an open source license, such as the MIT License or Apache License.
    \item The model must be accessible for modification and redistribution by users.
    \item The model must be accompanied by clear documentation and usage guidelines.
    \item The training data used to develop the model must be openly accessible or clearly specified.
\end{itemize}
